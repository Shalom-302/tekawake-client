    ...
    "relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground",
    {
        variants: {
            variant: {
                ...
                success:
                    "border-green-500/50 bg-green-50 text-green-700 dark:bg-green-900/50 dark:text-green-300 [&>svg]:text-green-500",
                warning:
                    "border-yellow-500/50 bg-yellow-50 text-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-300 [&>svg]:text-yellow-500",
...
export function Alert({
    ...
    if (children) {
        return (
            <ShadcnAlert className={cn(alertVariants({ variant, size }), className)} {...props}>
                {showIcon && getIcon()}
                {children}
            </ShadcnAlert>
        );
