      ...
      <div className="grid gap-8 mb-8">
        {isLoading ? (
          ...
          <div className="bg-yellow-50 border border-yellow-100 p-4 rounded-md">
            <p className="text-yellow-800">Impossible de récupérer les informations sur les cookies.</p>
          </div>
        ) : (
          <>
            <Card>
              ...
              <CardContent>
                <div className="flex items-center mb-4">
                  <Badge className={`mr-2 ${status.active ? "bg-green-500" : "bg-gray-400"}`}>
                  ...
                  {status.consent?.consentDate && (
                    <span className="text-sm text-gray-500">
                      Mis à jour le {new Date(status.consent.consentDate).toLocaleDateString()} à {new Date(status.consent.consentDate).toLocaleTimeString()}
                    </span>
                  )}
                </div>
                    ...
                    <div>
                      ...
                      <ul className="list-disc pl-5 space-y-1">
                        {status.consent.acceptedCookies.length > 0 ? (
                          ...
                          <li className="text-gray-500">Aucun cookie accepté</li>
                        )}
                      </ul>
                    </div>
                    
                    <div>
                      <h3 className="text-lg font-medium mb-2">Détails du consentement</h3>
