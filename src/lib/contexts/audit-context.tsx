...
export function AuditProvider({ children, userId }: AuditProviderProps) {
  ...
  const createAuditLog = useCallback(async (data: CreateAuditLogRequest): Promise<AuditLog> => {
  ...
  const logAction = useCallback(async (action: string, resource: string, details?: string): Promise<void> => {
    try {
      ...
      console.error(`Erreur lors de la journalisation de l'action ${action} sur ${resource}:`, error)
  ...
  const logCreate = useCallback(async (resource: string, resourceId: string | number): Promise<void> => {
    await logAction('CREATE', resource, `Created ${resource} with ID: ${resourceId}`)
  }, [logAction])
  
  const logUpdate = useCallback(async (resource: string, resourceId: string | number, details?: string): Promise<void> => {
    await logAction('UPDATE', resource, details || `Updated ${resource} with ID: ${resourceId}`)
  }, [logAction])
  
  const logDelete = useCallback(async (resource: string, resourceId: string | number): Promise<void> => {
    await logAction('DELETE', resource, `Deleted ${resource} with ID: ${resourceId}`)
  }, [logAction])
  
  const logView = useCallback(async (resource: string, resourceId: string | number): Promise<void> => {
