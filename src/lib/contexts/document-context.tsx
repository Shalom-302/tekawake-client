...
interface DocumentContextType {
    ...
    verifySignature: (
        documentId: string,
        signatureId: string
    ) => Promise<SignatureVerifyResponse | null>;
...
export const DocumentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const documentUpdateUnsubscribe = wsService.addDocumentUpdateHandler(message => {
            ...
            toast.info(
                `Document "${documents.find(d => d.id === message.document_id)?.name}" has been updated`
            );
        });
        ...
        const workflowStatusUnsubscribe = wsService.addWorkflowStatusHandler(message => {
            toast.info(
                `Document workflow status changed: ${message.current_step} → ${message.next_step || "completed"}`
            );
    ...
    const addSignatory = useCallback(
        async (documentId: string, email: string, name: string, order: number) => {
            setIsLoading(true);
            setError(null);
            try {
    ...
    const createTimestamp = useCallback(
        async (documentId: string, description: string) => {
            setIsLoading(true);
            setError(null);
            try {
