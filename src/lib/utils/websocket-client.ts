...
export class WebSocketClient {
    ...
    constructor(
        ...
        isGlobalConnection: boolean = false
    ) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (this.isGlobalConnection) {
                    ...
                    console.log(
                        `Connecting to global WebSocket: ${url.substring(0, url.indexOf("?"))}`
                    );
                } else {
                    ...
                    console.log(
                        `Connecting to conversation WebSocket: ${url.substring(0, url.indexOf("?"))}`
                    );
                }
                ...
                this.ws.onopen = event => {
                    console.log(
                        `WebSocket connected ${this.isGlobalConnection ? "(global)" : "(conversation " + this.conversationId + ")"}`
                    );
                ...
                this.ws.onmessage = event => {
                    try {
                        ...
                        if (this.options.debug) {
                            console.log(
                                `Received WebSocket message ${this.isGlobalConnection ? "(global)" : "(conversation)"}:`,
                                data.type
                            );
                        }
                ...
                this.ws.onclose = event => {
                    console.log(
                        `WebSocket closed ${this.isGlobalConnection ? "(global)" : "(conversation " + this.conversationId + ")"} with code: ${event.code}, reason: ${event.reason}`
                    );
    ...
    private reconnect(): void {
        ...
        console.log(
            `Attempting to reconnect (attempt ${this.reconnectAttempts + 1}/${this.options.maxReconnectAttempts})...`
        );
