...
const auditService = {
    ...
    async getAuditLogs(): Promise<AuditLog[]> {
        try {
            const response: AxiosResponse<AuditLog[]> =
                await axiosClient.get("/advanced_audit/logs");
    ...
    async getAuditLogsByResource(resource: string): Promise<AuditLog[]> {
        try {
            ...
            console.error(
                `Erreur lors de la récupération des logs d'audit pour la ressource ${resource}:`,
                error
            );
            throw error;
        }
    ...
    async getAuditLogsByAction(action: string): Promise<AuditLog[]> {
        try {
            ...
            console.error(
                `Erreur lors de la récupération des logs d'audit pour l'action ${action}:`,
                error
            );
            throw error;
        }
    ...
    async createAuditLog(logData: CreateAuditLogRequest): Promise<AuditLog> {
        try {
            const response: AxiosResponse<AuditLog> = await axiosClient.post(
                "/advanced_audit/logs",
                logData
            );
            return response.data;
        ...
        try {
            ...
            const metrics: AuditMetrics = {
                ...
                last_event_timestamp:
                    logs.length > 0 ? new Date(logs[0].created_at).getTime() / 1000 : null,
            };
