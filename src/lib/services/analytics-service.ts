...
const AnalyticsService = {
    ...
    async init(userId?: string): Promise<SessionInfo | null> {
        try {
            ...
            const response = await axiosClient.post<SessionInfo>(
                "/user-analytics/sessions/session",
                {
                    user_id: userId,
    ...
    async endSession(): Promise<void> {
        ...
        try {
            await axiosClient.post(
                `/user-analytics/sessions/session/${this.currentSession.id}/end`,
                {},
                {
                    headers: {
    ...
    async generateHeatmap(filter: HeatmapFilter): Promise<HeatmapData> {
        try {
            const response = await axiosClient.post<HeatmapData>(
                "/user-analytics/analytics/heatmap",
                filter
            );
        ...
        window.addEventListener("beforeunload", () => {
            ...
            if (this.pendingEvents.length > 0) {
                ...
                navigator.sendBeacon(
                    `${process.env.NEXT_PUBLIC_API_URL || ""}/user-analytics/events/batch`,
                    payload
                );
                this.pendingEvents = [];
            }
    ...
    _trackClick(event: MouseEvent): void {
        ...
        const componentName = elementToTrack
