...
export function useConversations() {
    ...
    } = useSWR<Conversation[]>(KEYS.conversations, () => API.fetchAPI(KEYS.conversations), {
    ...
    const getKey = useCallback(
        (pageIndex: number, previousPageData: Message[] | null) => {
            ...
            return [
                ...
                { limit: initialLimit, before: beforeId },
            ];
        },
    ...
    const sendMessage = useCallback(
        async (data: Omit<MessageCreate, "conversation_id">) => {
            ...
            try {
                ...
                setError(
                    err instanceof Error ? err : new Error("Erreur lors de l'envoi du message")
                );
                throw err;
            } finally {
                setIsLoading(false);
...
export function useUpdateMessage(conversationId: string) {
    ...
    const updateMessage = useCallback(
        async (messageId: string, data: MessageUpdate) => {
            setIsLoading(true);
            setError(null);

            try {
                const updatedMessage = await API.fetchAPI(
                    API.API_ROUTES.MESSAGE(conversationId, messageId),
                    {
                        method: "PATCH",
            ...
            try {
                ...
                switch (message.type) {
                    ...
                    case WebSocketMessageType.READ_RECEIPT:
                        ...
                        const receiptData = message.data as {
                            id: string;
...
export function useDeleteMessage() {
    const deleteMessage = useCallback(async (messageId: string) => {
        try {
            ...
            mutate(key => Array.isArray(key) && key[0] === `/conversations/${messageId}/messages`);
