...
export function UserJourneyViewer({
  ...
  const fetchUserJourney = async () => {
    try {
      ...
      setError(err.message || 'Une erreur est survenue lors de la récupération du parcours utilisateur');
    ...
    <Card className={cn("w-full", className)}>
      <CardHeader>
        <div className="flex flex-row items-center justify-between">
          ...
          <Button variant="default" size="sm" onClick={fetchUserJourney} disabled={loading}>
      ...
      <CardContent>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64">
            ...
            <p className="text-sm text-muted-foreground">Récupération des données en cours...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center h-64">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {format(new Date(session.start_time), "dd/MM/yy", { locale: fr })}
                        </div>
                      </div>
              ...
              {selectedSession ? (
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <div>
                      <h3 className="text-lg font-medium">Détails de session</h3>
                      <p className="text-sm text-muted-foreground">
                        {format(new Date(selectedSession.start_time), "PPpp", { locale: fr })}
                      </p>
                    </div>
                          ...
                          <div className="flex-1">
                            ...
                            {event.metadata && Object.keys(event.metadata).length > 0 && (
                              <div className="mt-1 p-2 rounded bg-muted text-xs">
                                <div className="font-medium mb-1">Métadonnées:</div>
                                {Object.entries(event.metadata).map(([key, value]) => (
                                  <div key={key} className="grid grid-cols-12 gap-1">
                                    <span className="col-span-4 text-muted-foreground">{key}:</span>
