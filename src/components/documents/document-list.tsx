...
export function DocumentList({ listType = 'grid', showFilters = true }: DocumentListProps) {
  ...
  const getStatusBadge = (status: DocumentStatus) => {
    switch (status) {
      ...
      case DocumentStatus.COMPLETED:
        return <Badge variant="success" className="bg-green-600">Complété</Badge>;
      case DocumentStatus.REJECTED:
        return <Badge variant="destructive">Rejeté</Badge>;
      case DocumentStatus.EXPIRED:
        return <Badge variant="outline" className="bg-amber-100 text-amber-800 border-amber-200">Expiré</Badge>;
      default:
  ...
  const getWorkflowStep = (step: WorkflowStep) => {
    switch (step) {
      case WorkflowStep.PREPARATION:
        return <Badge variant="outline" className="bg-blue-50 text-blue-800 border-blue-200">Préparation</Badge>;
      case WorkflowStep.SIGNATURE:
        return <Badge variant="outline" className="bg-purple-50 text-purple-800 border-purple-200">Signature</Badge>;
      case WorkflowStep.VERIFICATION:
        return <Badge variant="outline" className="bg-amber-50 text-amber-800 border-amber-200">Vérification</Badge>;
      case WorkflowStep.ARCHIVING:
        return <Badge variant="outline" className="bg-green-50 text-green-800 border-green-200">Archivage</Badge>;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="space-y-4">
      {showFilters && (
        <Tabs defaultValue="all" className="w-full" onValueChange={(value) => setFilter(value as DocumentStatus | 'all')}>
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    ...
                    <CardDescription className="text-xs">
                      Modified on {format(new Date(document.updated_at), 'dd/MM/yyyy')}
                    </CardDescription>
                  </div>
            ...
            <TableBody>
              {filteredDocuments.map((document) => (
                <TableRow key={document.id}>
                  ...
                  <TableCell>{format(new Date(document.created_at), 'dd/MM/yyyy')}</TableCell>
                  <TableCell>{format(new Date(document.updated_at), 'dd/MM/yyyy')}</TableCell>
